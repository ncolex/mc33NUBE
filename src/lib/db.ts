...
// Check if running on Vercel with PostgreSQL
 ...
 * Uses SQLite for local development, PostgreSQL for Vercel deployment
 */
export function getDatabase(): Database.Database {
  // If DATABASE_URL is set, we're on Vercel - use PostgreSQL
  if (isVercel) {
    throw new Error(
      'getDatabase() is for SQLite only. ' +
      'For PostgreSQL (Vercel), use getPostgresClient() from db-postgres.ts'
    );
  }
...
/**
 * Check if using PostgreSQL (Vercel mode)
 */
